...
# --- SOLID Repository Classes for compatibility with router/kasa.py ---
...
class HesapRepository:
    ...
    def get_all(self, tur: Optional[str] = None) -> List[Dict]:
        ...
        cur = conn.cursor()
        if tur:
            cur.execute('SELECT * FROM kasa_banka_hesap WHERE tur = ? ORDER BY id ASC', (tur,))
        else:
            cur.execute('SELECT * FROM kasa_banka_hesap ORDER BY id ASC')
...
# --- iraybay's custom functions kept intact ---
...
def get_banka_hesaplari(tur=None):
    ...
    try:
        ...
        if tur:
            cursor.execute("SELECT * FROM kasa_banka_hesap WHERE tur = ? ORDER BY id ASC", (tur,))
        else:
            cursor.execute("SELECT * FROM kasa_banka_hesap ORDER BY id ASC")
